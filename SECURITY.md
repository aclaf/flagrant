...
1. Navigate to the repository's [Security tab](https://github.com/aclaf/flagrant/security)
2. Click ["Report a vulnerability"](https://github.com/aclaf/flagrant/security/advisories/new)
...
- Open a [private security advisory](https://github.com/aclaf/flagrant/security/advisories/new) on GitHub
...
Environment:
- Flagrant version/commit: [version or commit hash]
...
I consider security issues in the following areas in scope for the library itself:

- **Argument parsing vulnerabilities**: Injection attacks, escaping issues, or malformed input handling in the parser
- **Unsafe API design**: Library APIs that make it easy for developers to introduce vulnerabilities in their applications
- **Path handling**: Path traversal vulnerabilities in library-provided file operations (if any)
- **Terminal output**: Terminal control sequence injection or ANSI escape sequence vulnerabilities in library code
- **Dependency vulnerabilities**: Security issues in direct dependencies that affect this library
- **Unsafe defaults**: Library configurations or behaviors that are insecure by default
- **Code execution paths**: Unintended code execution through library capabilities
- **Memory safety**: Issues in any compiled extensions (if applicable)
- **Cryptographic usage**: Misuse of cryptography or weak random number generation in library code

> [!NOTE]
> Vulnerabilities in applications built with this library are generally out of scope unless they stem from a library design flaw or insecure default.
...
- Issues in user applications built with this library (unless directly caused by library bugs)
- Denial of service through resource exhaustion in user applications (unless caused by a library bug that makes resource limits impossible to use)
- Terminal emulator-specific bugs not caused by this library
...
- **Public discussions**: For general security questions, best practices, or policy clarifications, open a discussion in [GitHub Discussions](https://github.com/aclaf/flagrant/discussions)
